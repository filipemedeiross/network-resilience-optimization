...
if not DEBUG:
    # In development runserver serves app static files directly. In the
    # container WhiteNoise serves the collected, fingerprinted assets.
...
DATABASE_PATH = Path(
    os.getenv("DATABASE_PATH", str(BASE_DIR / "data" / "db.sqlite3"))
)
DATABASES = {
    "default": {
...
if SOLVER_TIME_LIMIT <= 0:
    raise ValueError("SOLVER_TIME_LIMIT deve ser positivo.")
if SOLVER_CLAIM_TTL_SECONDS <= SOLVER_TIME_LIMIT + 2:
    raise ValueError(
        "SOLVER_CLAIM_TTL_SECONDS deve superar SOLVER_TIME_LIMIT em mais de 2 segundos."
    )
if SOLVER_BUSY_RETRIES < 0:
    raise ValueError("SOLVER_BUSY_RETRIES nao pode ser negativo.")
INSTANCE_GENERATION_MAX_ATTEMPTS = _positive_int_env(
    "INSTANCE_GENERATION_MAX_ATTEMPTS",
    20,
)
